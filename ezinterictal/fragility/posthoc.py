...
def read_perturbation_result(deriv_path, source_basename, description):
    if description not in [DERIVATIVETYPES.ROWPERTURB_MATRIX.value,
...
def run_viz_heatmap(deriv_path, subject, figures_path,
                    desc,
                    excel_fpath=None, ignore_runs=None):
    ...
    if ignore_runs is not None:
        fpaths = [fpath for fpath in fpaths
                  if not f'run-{run}' in str(fpath) for run in ignore_runs]
...
if __name__ == '__main__':
    ...
    for subject in all_subjects:
        ...
        run_viz_heatmap(deriv_path=deriv_path, subject=subject,
                        desc=description,
                        figures_path=figures_path, excel_fpath=excel_fpath)
